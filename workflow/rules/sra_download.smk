...
def get_layouts(wildcards):
    ivals = []
    for i in samples[samples.index.str.contains('^.RR', regex=True, case=True)].index.tolist():
        checkpoint_output = checkpoints.get_layout.get(
            sample=i, **wildcards).output.outdir
        ivals.extend(glob_wildcards(os.path.join(checkpoint_output, "{layout}.info")).layout)
    ivals2 =[]
    for ival in ivals:
        if ival == 'PAIRED':
            ivals2.append('pe')
    ...
    layouts = expand(
        os.path.join(
            ...
            "{sample}.{seqmode}.tsv"),
        zip,
        sample=[i for i in samples[samples.index.str.contains('^.RR', regex=True, case=True)].index.tolist()],
        seqmode=ivals2
        )
    return layouts
